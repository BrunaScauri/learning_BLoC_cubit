...
void main() {
  ...
  group('createUser', () {
    test(
      'Should complete successfulyl when the status code is 200 or 201',
      () async {
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          throwsA(
            const ApiFailure(message: 'Invalid email address', statusCode: 400),
          ),
        );
  ...
  test('Should throw [APIException] when status code is not 200', () async {
    ...
    expect(
      () => methodCall(),
      throwsA(ApiFailure(message: tMessage, statusCode: 500)),
    );
